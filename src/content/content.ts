...
function createSubtitleContainer(): HTMLElement {
  ...
  container.style.zIndex = '10000'; // より高いzIndexを使用
  container.style.width = '100%';
  container.style.textAlign = 'center';
  container.style.color = 'white';
  container.style.textShadow = '0 0 3px black, 0 0 3px black, 0 0 3px black'; // より強いテキストシャドウ
  container.style.fontSize = `${settings.fontSize}px`;
  container.style.padding = '10px';
  container.style.backgroundColor = 'rgba(0, 0, 0, 0.5)'; // 半透明の背景
  
  if (settings.position === 'bottom') {
    container.style.bottom = '15%'; // 下部に表示
  } else {
    container.style.top = '15%'; // 上部に表示
  }
...
function observeSubtitles(): void {
  ...
  function findSubtitleElement() {
    for (const selector of subtitleSelectors) {
      ...
      if (element) {
        console.log(`Amazon Prime Translator: 字幕要素が見つかりました (${selector})`);
        return element;
      }
    }
    return null;
      ...
      if (mutation.type === 'childList' || mutation.type === 'characterData') {
        ...
        if (subtitleElement && subtitleElement.textContent) {
          ...
          if (originalText && originalText !== lastSubtitleText) {
            ...
            console.log(`Amazon Prime Translator: 字幕テキストを検出しました: "${originalText}"`);
  ...
  subtitleElement = findSubtitleElement();
  if (subtitleElement) {
    console.log('Amazon Prime Translator: 初期字幕要素が見つかりました。監視を開始します。');
    if (subtitleElement.textContent) {
      const originalText = subtitleElement.textContent.trim();
      if (originalText) {
        lastSubtitleText = originalText;
        console.log(`Amazon Prime Translator: 初期字幕テキスト: "${originalText}"`);
        translateSubtitle(originalText).then((translatedText) => {
          const container = document.getElementById('amazon-prime-translator-container') || createSubtitleContainer();
          displaySubtitles(originalText, translatedText, container);
        });
    ...
    console.log('Amazon Prime Translator: 初期字幕要素が見つかりません。ページ変更を待機しています。');
...
async function initialize(): Promise<void> {
  ...
  new MutationObserver(() => {
    if (location.href !== lastUrl) {
      ...
      if (isAmazonPrimeVideoPage()) {
        console.log('Amazon Prime Translator: URLが変更されました。再初期化します。');
  ...
  // 初期実行
  if (isAmazonPrimeVideoPage()) {
    console.log('Amazon Prime Translator: Amazon Primeビデオページを検出しました。');
    setTimeout(observeSubtitles, 2000);
  }
}

// Amazon Prime Videoのページかどうかを判定
function isAmazonPrimeVideoPage(): boolean {
  return (
